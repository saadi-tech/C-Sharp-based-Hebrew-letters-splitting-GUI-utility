...
namespace WindowsFormsApp3
{
    public partial class Form1 : Form
    {
        ...
        private List<int[]> estimate_words(List<int[]> letters)
        {   
            
            List<int[]> words = new List<int[]>();
            int midpoint,midpoint_upper,midpoint_lower;
            int difference = (int)this.difference_selector.Value;

            int x1,y1,x2,y2;

            int right_corner = letters[0][0]+letters[0][2];
            x1 = letters[0][0];
            y1 = letters[0][1];
            int left_corner = 0;
            ...
            for(int i = 1; i <letters.Count; i++)
            {
                ...
                left_corner = letters[i][0] + letters[i][2]; //right corner of the next letter...
                x2 = letters[i][0];
                y2 = letters[i][1];
                Console.WriteLine("Difference: {0}", Math.Abs(right_corner - left_corner));
                if(Math.Abs(right_corner - left_corner) > difference)
                {
                    ...
                    x1 = letters[i][0];
                    y1 = letters[i][1];
                    right_corner = letters[i][0] + letters[i][2];
            ...
            Console.WriteLine("Total words found: {0}", total_words);
            return words;
        }
